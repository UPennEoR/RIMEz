...
def random_power_law(S_min, S_max, alpha, size=1):
    ...
    a = S_min**alpha
    b = S_max**alpha
    return (a + (b-a)*r)**(1./alpha)

def generate_point_source_flux(Nsrc, F_min, F_max, gamma):
    # a distribution with dN/dS ~ S**(-gamma), with S in [F_min, F_max]
    return random_power_law(F_min, F_max, 1. - gamma, Nsrc)

def generate_point_source_catalog(Nsrc, seed, F_min=0.5, F_max=100., gamma=1.8):

    np.random.seed(seed)

    Flux_150 = generate_point_source_flux(Nsrc, F_min, F_max, gamma)

    RA = 2*np.pi * np.random.uniform(0., 1., Nsrc)
    codec = np.arccos(2.*np.random.uniform(0.,1.,Nsrc) - 1.)
    dec = np.pi/2. - codec
...
def sky_from_catalog(catalog, nu_axis):
    F_nu = catalog['Flux_150'][None, :] *(nu_axis[:, None]/150.)**catalog['spectral_indices'][None, :]
    RA = catalog['RA']
    dec = catalog['dec']
...
def spin0_spherical_harmonics(ell, theta, phi, delta):
    ...
    Y_elm = np.sqrt((2.*ell +1.)/4./np.pi) * (np.cos(phases) + 1j*np.sin(phases))
    ...
    for el in range(L):
        for m in range(-el, el+1):
            ...
            if m >= 0:
                ...
                ssht_flm[:,ssht_ind] = (-1.)**(m) * np.conj(np.exp(1j*m*np.pi)*hp_flm[:,hp_ind])
...
def diffuse_sky_model_from_GSM2008(nu_axis, smooth_deg=0., ssht_index=True):
    ...
    Jy_per_K = A_Jy * 2 * k_b * (nu_axis*1e6/c)**2.

    # R_g2c = hp.rotator.Rotator(coord=['G','C']).mat

    R_g2c = utils.get_galactic_to_gcrs_rotation_matrix()

    gsm8 = pygsm.GlobalSkyModel(freq_unit='MHz',basemap='haslam', interpolation='cubic')
...
def diffuse_sky_model(nu_axis, R_g2c=None, ssht_index=True, smth_deg=0.):
    ...
    if smth_deg != 0.:
        Ilm_init = hp.smoothalm(Ilm_init, fwhm=np.radians(smth_deg), pol=False, verbose=False, inplace=True)
