...
def make_sigma_tensor():
    ...
    sigma[:,:,0] = np.array([[1.,0],[0.,1.]], dtype=nb.complex128)
    sigma[:,:,1] = np.array([[1.,0.],[0.,-1.]], dtype=nb.complex128)
    sigma[:,:,2] = np.array([[0.,1.],[1.,0.]], dtype=nb.complex128)
    sigma[:,:,3] = np.array([[0., -1.j],[1.j, 0.]], dtype=nb.complex128)
    return sigma
...
#     alt = np.arcsin(q[2])
    alt = np.arctan(q[2]/np.sqrt(q[0]**2. + q[1]**2.))

    az = np.arctan2(q[1],q[0])
    inds = np.where(az < 0.)[0]
                    ...
                    for d in range(2):
                        for g in range(Nstokes):
                            if bsigma[b,c,g] == True:
                                ...
                                V[a,d] += F1[n] * J1[n,a,b] * sigma[b,c,g] * S[n,g] * J2_conj[n,d,c] * F2_conj[n]
    return V
...
def vec_psv_constructor(beam_funcs, compile_target='parallel'):
    @nb.guvectorize('float64[:,:], float64[:], float64[:,:],\
                       ...
                       nopython=True, fastmath=False, target=compile_target)
    def vec_psv(R_i, nu_axis, r_axis,
                ant_pairs, ant_ind2beam_func,
                S, RA, dec, V_i):
        ...
        #             phases = -2*np.pi*nu_j/c * np.dot(r_axis, s_v.T)
                ...
                J_arr = np.zeros((beam_func_indices.shape[0], alt_v.shape[0], 2, 2), dtype=nb.complex128)
                ...
                for k in range(Nbl):
                    ...
                    V_i[j,k] = RIME_sum(J_p, J_q_conj, F_p, F_q_conj, S_j_v, sigma, bsigma)

    return vec_psv

def parallel_point_source_visibilities(rotations_axis, nu_axis, r_axis,
                              ant_pairs, beam_funcs,
                              ant_ind2beam_func,
                              S, RA, dec):

    Nbl = ant_pairs.shape[0]
    V = np.zeros((rotations_axis.shape[0], nu_axis.shape[0], Nbl, 2, 2), dtype=np.complex128)

    vec_point_source_visibilities = vec_psv_constructor(beam_funcs)

    vec_point_source_visibilities(rotations_axis, nu_axis, r_axis,
                                  ant_pairs, ant_ind2beam_func,
                                  S, RA, dec, V)
...
def vec_muv_constructor(beam_funcs, compile_target='parallel'):
    @nb.guvectorize('float64, float64[:,:],\
                    ...
                    nopython=True, target=compile_target)
    def vec_mmode_unpol_visibilities(nu, r_axis,
                                    ant_pairs, ant_ind2beam_func,
                                    Slm, R_0, Ls, Lb, dummy_maxis, V_nu_m):
        ...
        J_arr = np.zeros((beam_func_indices.shape[0], alt.shape[0]) + (2,2), dtype=nb.complex128)
        for i_bf in range(beam_func_indices.shape[0]):
            index = beam_func_indices[i_bf]
            J_i = J_arr[index]
                            ...
                            for i_c in range(2):
                                for i_d in range(2):
                                    if bsigma[i_b,i_c,0] == True:
                                        h = i_a + 2*i_d
                                        i_rav = i_ph + ra.shape[1]*i_th # equiv to np.ravel_multi_index((i_th, i_ph), (Lb, 2*Lb-1), order='C')

                                        A_p_ab = F_arr[p, i_rav] * J_arr[ind_p, i_rav, i_a, i_b]
                                        A_q_conj_dc = np.conj(J_arr[ind_q, i_rav, i_d, i_c] * F_arr[q, i_rav])

                                        K_CI[i_a, i_d, i_th, i_ph] +=  A_p_ab * sigma[i_b,i_c,0] * A_q_conj_dc
                ...
                for i_b in range(2):
                    for m in range(-Lm, Lm+1):
                        ...
                        for el in range(abs(m), Lm):
                            ...
                            V_nu_m[k, i_m, i_a, i_b] += Klm_cI[i_a, i_b, index] * np.conj(Slm[index,0])

    return vec_mmode_unpol_visibilities

def parallel_mmode_unpol_visibilities(nu_axis, r_axis,
                                      ant_pairs, beam_funcs, ant_ind2beam_func,
                                      Slm, R_0, Ls, Lb):
    Nbl = ant_pairs.shape[0]
    if Ls < Lb:
        Lm = Ls
    ...
    vec_mmode_unpol_visibilities(nu_axis, r_axis,
                                ant_pairs, ant_ind2beam_func,
                                Slm, R_0, Ls, Lb, dummy_maxis, Vm)

    return Vm

@nb.njit
def mmode_unpol_visibilities(nu_axis, r_axis,
                             ant_pairs, beam_funcs, ant_ind2beam_func,
                             Slm, R_0, Ls, Lb):
    c = 299792458.
    sigma = make_sigma_tensor()
                ...
                for i_b in range(2):
                    for m in range(-Lm, Lm+1):
                        ...
                        for el in range(abs(m), Lm):
                            ...
                            Vm[i, k, i_m, i_a, i_b] += Klm_cI[i_a, i_b, index] * np.conj(Slm_i[index,0])
    return Vm
...
def visiblity_dft_from_mmodes(era_axis, Vm):
    ...
    """
    V_dft = np.zeros((era_axis.size,) + Vm.shape[:2] + Vm.shape[3:], dtype=nb.complex128)
...
#     phase_shift_up = np.exp(-1j*m_axis*ang_shift_up)
    ...
    for j in range(Vm.shape[0]):
        for k in range(Vm.shape[1]):
            for a in range(2):
                for b in range(2):
                    Vm_padded[j,k,ind0:ind1+1,a,b] = Vm[j,k,:,a,b] * phase_shift_up

    era_fftup_axis = 2*np.pi * np.arange(-(N_fftup-1)/2, (N_fftup-1)/2 + 1)/N_fftup
    era_fftup_axis += np.pi

    V_fftup = np.fft.fft(np.fft.ifftshift(Vm_padded, axes=2), axis=2)

    V_itp = np.zeros((era_axis.size,) + Vm.shape[:2] + Vm.shape[3:], dtype=np.complex128)
    ...
    for j in range(V_itp.shape[1]):
        for k in range(V_itp.shape[2]):
            ...
            tck,_ = interpolate.splprep(V_temp.T, u=per_era_in, k=3, s=0., full_output=0, per=1)
