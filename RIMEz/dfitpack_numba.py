...
def bispeu_nb(tx,ty,c,kx,ky,x,y):
    ...
    bispeu_wrap(ffi.from_buffer(tx), ffi.from_buffer(nx),
                ffi.from_buffer(ty), ffi.from_buffer(ny),
                ffi.from_buffer(c),
                ffi.from_buffer(kx), ffi.from_buffer(ky),
                ffi.from_buffer(x), ffi.from_buffer(y),
                ffi.from_buffer(z), ffi.from_buffer(m),
                ffi.from_buffer(wrk), ffi.from_buffer(lwrk),
                ffi.from_buffer(ier))
