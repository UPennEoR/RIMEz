...
def model_data_to_spline_beam_func(full_file_name, nu_axis, L_synth=180, indexed=False):
    ...
    spline_beam_func = construct_spline_beam_func(nu_axis, tx,ty,kx,ky,E_coeffs, rE_coeffs)
...
def construct_spline_beam_func(nu_axis, tx, ty, kx, ky, E_coeffs, rE_coeffs, imap='default'):
    """
    ...
    HERA_COLAT = 90. + 30.72152777777791
    R = rotation_matrix(np.array([0,1.,0]), np.radians(HERA_COLAT))
    ...
    def spline_beam_func(nu, alt, az):
        ...
        u = [-1., -1j] # negative for components in alt/az basis
#         u = [1., 1j]

        for kk in range(2):
            for aa in range(2):

                J_aa[...,imap[0], aa] += u[kk]*bispeu_nb(tx, ty, E_coeffs[i_nu, aa, kk, :], kx, ky, mu, phi)[0]
                J_aa[...,imap[1], aa] += u[kk]*bispeu_nb(tx, ty, rE_coeffs[i_nu, aa, kk, :], kx, ky, mu, phi)[0]
        ...
        tukeyW = tukey_window(alt, np.radians(2.))
...
def cartENU2altaz(E,N,U):
    alt = np.arctan(U/np.sqrt(E**2. + N**2.))
    az = np.arctan2(E, N)
    inds = np.where(az < 0.)[0]

    az[inds] = az[inds] + 2.*np.pi
    return alt, az
...
def az_shiftflip(az_in):
    ...
    """
    az_out = np.pi/2. - az_in
    az_out[az_out < 0.] += 2*np.pi
    return az_out
